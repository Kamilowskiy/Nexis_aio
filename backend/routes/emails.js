...
export default function emailRoutes(oauth2Client, userTokens) {
  ...
  router.get('/', requireAuth, async (req, res) => {
    try {
      ...
      // LIMIT: Maximum 200 wiadomości per request (optymalizacja)
      const limitedMaxResults = Math.min(parseInt(maxResults), 200);
  ...
  // 1.5. Get TODAY's emails stats (for counters)
  router.get('/stats/today', requireAuth, async (req, res) => {
    try {
      ...
      // Query dla emaili z dzisiaj
      const query = `after:${todayStartTimestamp}`;
      
      const response = await req.gmail.users.messages.list({
        userId: 'me',
        maxResults: 500, // Limit dla dzisiejszych (powinno wystarczyć)
        q: query,
        labelIds: ['INBOX']
      });

      const todayMessages = response.data.messages || [];
      
      // Count unread from today
      const unreadToday = await Promise.all(
        todayMessages.slice(0, 100).map(async (message) => {
          try {
            const details = await req.gmail.users.messages.get({
              userId: 'me',
              id: message.id,
            ...
            return details.data.labelIds?.includes('UNREAD') ? 1 : 0;
          } catch (err) {
            return 0;
          }
        })
      ...
      res.json({
        ...
        unreadToday: unreadToday.reduce((sum, val) => sum + val, 0),
